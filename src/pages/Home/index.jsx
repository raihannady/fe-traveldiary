      ...
      <Box className={classes.container}>
        <Box className={classes.content}>
          <Box className={classes.title}>
            {/* <Typography>{login ? 'Hello, ' : ''}</Typography> */}
            <Typography variant="h4">Journey</Typography>
          </Box>
          <Box className={classes.search}>
            {/* <Stack direction="row" sx={{ width: 300 }} fullWidth> */}
            <Autocomplete
              fullWidth
              id="free-solo-demo"
              freeSolo
              options={top100Films.map((option) => option.title)}
              renderInput={(params) => <TextField {...params} label="Find Journey" variant="filled" size="small" />}
            />
          ...
          {post?.map((item) => {
            return (
              <Box className={classes.cardWrapper} key={item.id} href={`/detail/${item.id}`}>
                <Link to={`/detail/${item.id}`}>
                  <Card sx={{ width: 300, height: 362 }} className={classes.card}>
